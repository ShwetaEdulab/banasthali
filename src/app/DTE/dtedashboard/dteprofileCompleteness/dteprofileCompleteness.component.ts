...
@Component({
  selector: 'ngx-solar',
  styleUrls: ['./dteprofileCompleteness.component.scss'],
  template: `
    <nb-card status="success" size="xsmall" class="solar-card">
     <nb-card-header><h2>Your Profile Completeness</h2></nb-card-header>
      <nb-card-body>
        <a href="https://mu.admissiondesk.org/app/#/pages/DTEProfile"><div echarts [options]="option" class="echart">
        </div></a>
        <div class="info">
          <div class="value">{{ value }} </div>
          <div class="details" *ngIf="transcriptLock == false"><span>out of</span> 100 %</div>
          <div class = "row" *ngIf="value<100">
            <a (click)="onprofile()"><span style="color:red; font-size: 100%;"><u><b><br>click here to fill your profile.</b></u></span></a>          
          </div>
...
//   }
 ...
 ngOnInit() {
  ...
  onClickTranscript(){
    this.router.navigate(['pages/DTEProfile'],{queryParams:{selectedIndex:4}});
  }
