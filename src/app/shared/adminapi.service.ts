...
getApplicationAcceptReject(tab_type,year){
  try{
    return this.httpClient.get(`${this.baseUrl}/admin_api/Eligibility/acceptreject?tab=`+tab_type+`&year=`+year);
  }catch(error) {
      this.handleError("getApplicationAcceptReject : "+JSON.stringify(error));
  }
}

getDteApplicationAcceptReject(tab_type,year){
  try{
    return this.httpClient.get(`${this.baseUrl}/admin_api/Eligibility/Dteacceptreject?tab=`+tab_type+`&year=`+year);
  }catch(error) {
      this.handleError("getApplicationAcceptReject : "+JSON.stringify(error));
  }
}
...
async getAllDTEStudentData(userId){
  try{
      return await this.httpClient.get(`${this.baseUrl}/admin_api/View/DTEstudentview?userId=`+userId).toPromise();
  }catch(error) {
  this.handleError("getAllDTEStudentData : "+error);
  }
}

dte_preview(userId,courseId,id){
  try{
    return this.httpClient.post(`${this.baseUrl}/admin_api/Application/Dte_preview`,{userId:userId,courseId:courseId,id:id});
  }catch(error) {
...
getallstudents(year,FromDate,ToDate,country_id){
  try{
    return this.httpClient.get(`${this.baseUrl}/admin_api/students?year=`+year+'&FromDate='+FromDate+'&ToDate='+ToDate+'&country_id='+country_id);
  }catch(error) {
...
showCollegeAllocatedExcel(year,collegename){
  try{
    return this.httpClient.get(`${this.baseUrl}/admin_api/Eligibility/showCollegeAllocatedExcel?year=`+year+`&collegename=`+collegename);
  }catch(error) {
...
getCountryWiseStudent(year){
  try{
    return this.httpClient.get(`${this.baseUrl}/admin_api/report/getCountryWiseRegisterStudents?year=`+year);
  }catch(error){
    this.handleError("getCountryWiseStudent :"+JSON.stringify(error));
  }
}

async getAllStudentStepperData(userId,courseId,id,category){
  try{
      return await this.httpClient.get(`${this.baseUrl}/admin_api/View/studentStepper?userId=`+userId+`&courseId=`+courseId+`&id=`+id+`&category=`+category).toPromise();
  }catch(error) {
  this.handleError("getAllStudentStepperData : "+error);
  }
}

getUTMSourceData(source){
  try{
    return this.httpClient.get(`${this.baseUrl}/admin_api/report/utm_source?utm_source=`+source);
  }catch(error) {
      this.handleError("getUTMSourceData : "+JSON.stringify(error));
  }
}

thirdPaymentExcel(year){
  try{
    return this.httpClient.get(`${this.baseUrl}/admin_api/foreignoffice/thirdPaymentExcel?year=`+year);
  }catch(error){
    this.handleError("thirdPaymentExcel : "+JSON.stringify(error));
  }
}

get3rdPaymentDetails(user_id){
  try{
    return this.httpClient.post(`${this.baseUrl}/admin_api/foreignoffice/get3rdPaymentDetails`,{
      user_id : user_id
    });
  }catch(error) {
      this.handleError("get3rdPaymentDetails : "+JSON.stringify(error));
  }
}

getRejectedStudents(){
  try{
    return this.httpClient.get(`${this.baseUrl}/admin_api/dashboard/rejectedStudents`);
  }catch(error) {
      this.handleError("getRejectedStudents : "+JSON.stringify(error));
  }
}

getUserInformation(user_id){
  try{
    return this.httpClient.get(`${this.baseUrl}/admin_api/View/getUserInformation?userId=`+user_id).toPromise();
  }catch(error) {
      this.handleError("getUserInformation : "+JSON.stringify(error));
  }
}
