...
getProfileCompleteness(){
  ...
  trackerChecktab(id){
    try{
      return this.httpClient.get(`${this.baseUrl}/api/myApplication/trackerChecktab?id=`+id);
    }catch(error){
...
//Cart 
  ...
  SecondPaymentEmail(appId){
    try{
      return this.httpClient.get(`${this.baseUrl}/api/cron/Second_Pay_Email?appId=`+appId).toPromise();
    }catch(error){
      this.handleError("SecondPaymentEmail : "+error);
    }
...
updateCalleeURL(id){
  try{
    return this.httpClient.post(`${this.baseUrl}/updateCalleeURL`,{
      "id" : id
    });
  }catch(error) {
    this.handleError("updateCalleeURL : "+error);
  }
}


check1stpayment(courseId,appId){
  try{
    return this.httpClient.get(`${this.baseUrl}/api/payment/check1stpayment?courseId=`+courseId+`&appId=`+appId);
  }
  catch(error) {
    this.handleError("check2ndpayment: "+JSON.stringify(error));
  }
}

getFees(courseId){
  try{
    return this.httpClient.post(`${this.baseUrl}/api/payment/getFees`,{"courseId":courseId});
  }catch(error) {
    this.handleError("getFees : "+error);
  }
}
