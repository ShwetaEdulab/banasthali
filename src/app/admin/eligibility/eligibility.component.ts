...
export class AdminEligibilityComponent {
  ...
  DownloadApplicationLetter(userId,courseId,applicationId,user_type){
    if(user_type == 'dte'){
    
      this.adminApi.dte_preview(userId,courseId,applicationId).subscribe(data => {
        if(data[`status`] == 200){
          this.adminApi.downloadFiles(data[`data`])
          .subscribe(data => {
            saveAs(data, applicationId+'_Preview.pdf');    
  ...
  getApplication(e) {
    ...
    if(this.tab_type == 'new'){
      ...
      this.adminApi.getApplicationAcceptReject(this.tab_type,this.selectedYear).subscribe(data=>{
        this.application_data = data['data'];
      })
      this.filterText = "";
      this.filterPlaceholder = "Search";
    ...
    }else if(this.tab_type === 'dte_accepted'){
      this.adminApi.getDteApplicationAcceptReject(this.tab_type,this.selectedYear).subscribe(data=>{
        this.application_data = data['data'];
      })
      this.filterText = "";
      this.filterPlaceholder = "Search";
  ...
  viewMore(category,userId,courseId,tab,applicationId,user_type){
    this.router.navigate(['pages/adminView'],{queryParams:{category:category,userId : userId, courseId:courseId,tab:tab,applicationId:applicationId,user_type:user_type }});
  }
