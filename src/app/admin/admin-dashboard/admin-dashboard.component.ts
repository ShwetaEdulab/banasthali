  	...
  	ngOnInit() {
    	...
    	this.adminApi.profile_completeness(this.selectedYear).subscribe(data=>{
				if(data['data']['data'].length > 0){
					...
					this.Applied = parseInt(data['data']['data'][0]['applied']);
					this.Count = data['data']['count']
					this.data = {
						
						labels: ['10%','20%', '40%', '60%', '80%', '100%','Applied'],
						datasets: [
							{
								label: 'Profile Completeness',
								backgroundColor: " #2e9e73",
								hoverBackgroundColor:"#3a5b3c",                      
								data: [ this.Ten, this.Twenty,this.Fourty,  this.Sixty,  this.Eighty,  this.Hundred, this.Applied]
							},
						]
					}
				}
  	...
  	tab(e) {
		...
		if(index == 0){
          ...
          this.length = 0;
			...
			this.tab_type = "RejectedStudents_SPPU";
			this.loadingbutton = true;
			this.adminApi.getRejectedStudents().subscribe(data =>{
				this.loadingbutton = false;
