...
export class AdminReportComponent {
  ...
  filterYearStudent(year){
    this.adminApi.getCountryWiseStudent(year).subscribe(data =>{
      var label = [];
      var count = [];
      this.StudentcountryData = data['data'];
      for(var a in data['data']){
        label.push(data['data'][a]['country']);
        count.push(data['data'][a]['count']);
      }
    ...
    if(index == 0){
     ...
     this.filterInput
      ...
      this.adminApi.studentStatisticsPiechart().subscribe(data=> {
        ...
        var label = [];
        var count = [];
        this.countryData = data['data']['counts'];
				for(var a in data['data']['counts']){
					label.push(data['data']['counts'][a]['country']);
					count.push(data['data']['counts'][a]['count']);
				}
				this.pieChartLabels = label;
				this.pieChartData = [
					{ 
      ...
      this.adminApi.getCountryWiseStudent(2019).subscribe(data =>{
        var label = [];
        var count = [];
        this.StudentcountryData = data['data'];
        for(var a in data['data']){
      ...
      this.adminApi.getUTMSourceData(this.selectedSource).subscribe(data=>{
  ...
  sourcefilter(source){
    this.adminApi.getUTMSourceData(source).subscribe(data=>{
      this.studentdata = data['data'];
    })
    this.filterText = "";
    this.filterPlaceholder = "Search";
