...
export class PagesComponent {
  ...
  var checkDteapplications =  this.dteApi.checkDteapplications();
  checkDteapplications.subscribe(
    (data: any) => {
      if(data['status']== 200){
        this.show = 'true';
        this.showDteMyApplication();
      }else if(data['status']== 400){
        this.show = 'false';
        this.showDteMyApplication();
      }else{
...
showDteMyApplication(){
  for (var a in MENU_ITEMS){
    if(MENU_ITEMS[a].title == 'Application'){
      if(this.show == 'false'){
        MENU_ITEMS[a].hidden = true;
      }else{
        MENU_ITEMS[a].hidden = false;
